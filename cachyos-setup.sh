...
configure_reflector() {
    ...
    print_status "Creating reflector configuration..."
    cat > /etc/xdg/reflector/reflector.conf << 'EOF'
# Reflector configuration file
# Auto-generated by CachyOS setup script

# Sort mirrors by download rate
--sort rate

# Use mirrors from Germany, Sweden, and Denmark
--country DE,SE,DK

# Limit to latest 10 mirrors
--latest 10
...
EOF
    ...
    # Create systemd timer override for daily execution at 18:00
    print_status "Setting up daily reflector timer at 18:00..."

    mkdir -p /etc/systemd/system/reflector.timer.d
    cat > /etc/systemd/system/reflector.timer.d/override.conf << 'EOF'
[Timer]
# Clear default timer settings
OnCalendar=
OnBootSec=
...
EOF
    ...
    # Enable and start reflector service and timer
    systemctl daemon-reload

    # Run reflector once now to update mirrors
    print_status "Running reflector to update mirror list..."
    reflector --sort rate --country DE,SE,DK --latest 10 --protocol https --save /etc/pacman.d/mirrorlist
